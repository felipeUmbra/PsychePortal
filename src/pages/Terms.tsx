import { motion } from 'motion/react';
import { useNavigate } from 'react-router-dom';
import { ArrowLeft, ShieldAlert } from 'lucide-react';
import { useTranslation } from 'react-i18next';

export default function Terms() {
  const navigate = useNavigate();
  const { t } = useTranslation();

  return (
    <div className="min-h-screen bg-bg py-12 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-3xl mx-auto">
        <button 
          onClick={() => navigate('/')} 
          className="flex items-center gap-2 text-text-muted hover:text-text-main transition-colors mb-8 font-bold text-[14px]"
        >
          <ArrowLeft className="w-4 h-4" />
          {t('terms_page.back')}
        </button>
        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="bg-surface rounded-2xl border border-border-custom p-8 sm:p-10 shadow-sm"
        >
          <div className="flex items-center gap-4 mb-6 pb-6 border-b border-border-custom">
            <div className="w-12 h-12 bg-primary-custom/10 rounded-xl flex items-center justify-center text-primary-custom shrink-0">
              <ShieldAlert className="w-6 h-6" />
            </div>
            <div>
              <h1 className="text-3xl font-bold text-text-main">
                {t('terms_page.title')}
              </h1>
              <p className="text-[12px] text-text-muted font-semibold uppercase tracking-wider mt-1">
                {t('terms_page.subtitle')}
              </p>
            </div>
          </div>

          <div className="space-y-8 text-[15px] text-text-muted leading-relaxed">
            <p className="text-lg text-text-main font-medium">
              {t('terms_page.intro')}
            </p>

            <div className="bg-red-500/5 border border-red-500/10 p-6 rounded-xl">
              <h2 className="text-text-main font-bold text-base mb-2">
                {t('terms_page.beta_disclaimer_title')}
              </h2>
              <p className="text-sm">
                {t('terms_page.beta_disclaimer_text')}
              </p>
            </div>

            <div className="space-y-3">
              <h2 className="text-text-main font-bold text-lg">
                {t('terms_page.user_responsibility_title')}
              </h2>
              <p>
                {t('terms_page.user_responsibility_text')}
              </p>
            </div>

            <div className="space-y-3">
              <h2 className="text-text-main font-bold text-lg">
                {t('terms_page.liability_title')}
              </h2>
              <p>
                {t('terms_page.liability_text')}
              </p>
            </div>

            <div className="space-y-3">
              <h2 className="text-text-main font-bold text-lg">
                {t('terms_page.accountability_title')}
              </h2>
              <p>
                {t('terms_page.accountability_text')}
              </p>
            </div>
          </div>
        </motion.div>
      </div>
    </div>
  );
}
